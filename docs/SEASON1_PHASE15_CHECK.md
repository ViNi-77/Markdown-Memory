...
この自動確認で証明できるのは、Xcode projectの構文、target認識、iOS Simulator向けDebug build、Simulatorでのアプリ起動です。Googleログイン、保存、共有、AIパネル表示は、SimulatorまたはiPhone実機での手動確認を完了条件として扱います。
...
- PR #74: Phase 15自動確認状況の記録
- PR #75: Simulator smoke timeout安定化
- main CI: `27858559535` success
- main iOS Shell: `27858559553` success
- Vercel Production deployment: commit `3581fac51d724b06e60cc63821c6f425847e9f7b` で success
...
ユーザーによるiOS shellの手動スモーク確認で、以下を確認済みです。

- Production URLがiOS shell内で開ける
- Googleログイン後のワークスペース表示
- Markdown作成、保存、リロード後復元
- 共有リンク作成、未ログイン閲覧、共有解除
- アプリ内AIパネル表示とProvider切替
- 外部リンク、戻る導線、Safe Areaに重大な崩れがない
- Web/PWA版と比べて重大な劣化がない

## 再確認手順

Phase 15 の手動確認を再現するには、iOS shell上でログイン後の主要導線を確認します。Xcodeをインストールまたは選択した環境では、以下で同じshellを起動できます。
...
2026-06-20 時点で、Phase 15の自動確認と手動確認は完了です。Season 1は次にPhase 16へ進み、Apple Developer Team設定、Archive、App Store Connect登録、TestFlight uploadへ進みます。
