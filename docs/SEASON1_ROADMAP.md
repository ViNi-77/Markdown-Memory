...
Season 1 の最終ゴールは、Web/PWAとして完成度を高めたうえで、iOS TestFlight の内部配布まで到達することです。2026-06-25 に TestFlight 内部配布と iPhone 実機確認まで完了しました。App Store本番公開、App Store審査対策としてのiOS固有価値追加、Macアプリ化は Season 2 以降で扱います。
...
2026-06-25 に Phase 16 は完了しました。Apple Developer Program加入、App Store Connect登録、Xcode Archive、App Store Connect upload、TestFlight内部配布、iPhone実機確認まで到達しています。TestFlight build は `1.0.0 (1)` です。実機確認後に気になったモバイル下部ナビの切替表示は PR #91 で修正し、Productionへ反映済みです。詳細は [`SEASON1_PHASE16_CHECK.md`](SEASON1_PHASE16_CHECK.md) に記録します。
